...
# Database ID for pharma dataset - required for SQL queries
...
DATASET_ID = "c5b6c049-64c9-4249-9e98-21b4f02f9aa1"
...
def run_forecast_analysis(parameters: SkillInput) -> SkillOutput:
    ...
    try:
        ...
        # NOTE: Price metric logic commented out for pharma (all metrics use SUM)
        # Uncomment below if you need AVG aggregation for price/rate metrics in the future
        ...
        is_price_metric = False  # Pharma uses SUM for all metrics

        # Format facts differently for price vs volume metrics
        if is_price_metric:
            # COMMENTED OUT - Price metric formatting (keep for future use)
            facts = {
                'metric': metric,
...
def fetch_data(metric, start_date, other_filters):
    ...
    # Use SUM aggregation for all pharma metrics
    agg_method = 'SUM'
    ...
    try:
        ...
        if raw_df is not None and not raw_df.empty:
            ...
            if 'month' in raw_df.columns:
                raw_df = raw_df.rename(columns={'month': 'period'})
                print(f"DEBUG: Renamed month to period")
