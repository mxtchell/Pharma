...
@skill(
    name="Pharmaceutical Market Intelligence",
    description="Retrieves and analyzes pharmaceutical industry reports and market intelligence to answer questions about global medicine trends, therapeutic areas, spending forecasts, and market dynamics",
    capabilities="Searches through pharmaceutical industry documents (IQVIA reports, market outlooks), finds relevant market trends, spending forecasts, therapeutic area analysis, and regional dynamics with citations and references",
...
def find_matching_documents(user_question, topics, loaded_sources, base_url, max_sources, match_threshold, max_characters):
    """Find documents using enhanced keyword matching optimized for pharmaceutical market intelligence"""
    ...
    try:
        ...
        # Pharmaceutical keyword expansions
        keyword_expansions = {
            'spending': ['spending', 'expenditure', 'cost', 'investment', 'budget', 'financial', 'revenue', 'sales'],
            'growth': ['growth', 'increase', 'expansion', 'rise', 'growing', 'trend', 'projection', 'outlook'],
            'therapeutic': ['therapeutic', 'therapy', 'treatment', 'indication', 'disease', 'condition', 'oncology', 'diabetes', 'cardiovascular'],
            'market': ['market', 'industry', 'sector', 'segment', 'regional', 'global', 'emerging', 'developed'],
            'medicine': ['medicine', 'drug', 'pharmaceutical', 'biologic', 'therapy', 'treatment', 'medication'],
            'specialty': ['specialty', 'branded', 'generic', 'biosimilar', 'innovative', 'novel'],
            'access': ['access', 'affordability', 'pricing', 'policy', 'reimbursement', 'coverage'],
            'rd': ['r&d', 'research', 'development', 'pipeline', 'innovation', 'launch', 'approval']
        }
        ...
        for source in loaded_sources:
            ...
            if score >= float(match_threshold):
                ...
                doc_id = "50f685d2-0617-4334-97d3-509b62d20095"
...
def calculate_simple_relevance(text, search_terms):
    """Enhanced relevance scoring for pharmaceutical market intelligence documents"""
    text_lower = text.lower()
    score = 0.0
    debug_matches = []

    # Domain-specific keywords for pharmaceutical intelligence
    spending_keywords = ['spending', 'expenditure', 'cost', 'investment', 'revenue', 'sales', 'billion', 'trillion', '$']
    growth_keywords = ['growth', 'increase', 'expansion', 'rise', 'growing', 'projected', 'forecast', 'outlook', 'trend']
    therapeutic_keywords = ['therapeutic', 'therapy', 'treatment', 'oncology', 'diabetes', 'cardiovascular', 'immunology', 'neurology']
    market_keywords = ['market', 'regional', 'global', 'emerging', 'developed', 'americas', 'europe', 'asia', 'china']
    medicine_keywords = ['medicine', 'drug', 'pharmaceutical', 'biologic', 'biosimilar', 'generic', 'branded', 'specialty']
    policy_keywords = ['policy', 'pricing', 'access', 'reimbursement', 'affordability', 'regulation', 'approval']

    # Check for domain-specific content first
    for keyword in spending_keywords:
        if keyword in text_lower:
            score += 0.15
            debug_matches.append(f"spending:{keyword}")

    for keyword in growth_keywords:
        if keyword in text_lower:
            score += 0.12
            debug_matches.append(f"growth:{keyword}")
    ...
    for keyword in medicine_keywords:
        if keyword in text_lower:
            score += 0.1
            debug_matches.append(f"medicine:{keyword}")
...
def calculate_enhanced_relevance(text, search_terms, file_name):
    """Enhanced relevance scoring optimized for pharmaceutical market intelligence"""
    text_lower = text.lower()
    score = 0.0

    # Critical pharma keywords with weights
    critical_keywords = {
        # Spending & Financial
        'spending': 0.5, 'expenditure': 0.4, 'billion': 0.6, 'trillion': 0.6, 'revenue': 0.4,
        'investment': 0.4, 'cost': 0.3, 'sales': 0.4,

        # Growth & Trends
        'growth': 0.5, 'projected': 0.4, 'forecast': 0.5, 'outlook': 0.4, 'trend': 0.4,
        'increase': 0.3, 'expansion': 0.3,

        # Therapeutic Areas
        'oncology': 0.5, 'diabetes': 0.5, 'cardiovascular': 0.5, 'immunology': 0.5,
        'therapeutic': 0.4, 'treatment': 0.3, 'therapy': 0.3,

        # Markets & Regions
        'global': 0.4, 'emerging': 0.4, 'market': 0.3, 'regional': 0.3,

        # Medicine Types
        'specialty': 0.5, 'biosimilar': 0.5, 'generic': 0.4, 'branded': 0.4,
        'biologic': 0.4, 'pharmaceutical': 0.3,

        # Policy & Access
        'policy': 0.4, 'pricing': 0.4, 'access': 0.4, 'reimbursement': 0.4,
        'affordability': 0.3, 'regulation': 0.3
    }
    ...
    # High-value pharma patterns
    high_value_patterns = [
        ('global medicine outlook', 0.8),
        ('therapeutic area', 0.7),
        ('emerging market', 0.7),
...
def generate_rag_response(user_question, docs):
    ...
    try:
        ...
        title = f"Pharmaceutical Market Intelligence: {user_question}"
        content = f"<p>Based on the pharmaceutical industry documents, here's the relevant information:</p>"
        ...
        for i, doc in enumerate(docs):
            ...
            if clean_text and len(clean_text) > 20:
                ...
                for line in lines:
                    ...
                    # Extract lines with important pharma information
                    if any(keyword in line_lower for keyword in ['spending', 'growth', 'billion', 'trillion', 'therapeutic', 'market', 'specialty', 'generic', 'biosimilar', 'policy', 'forecast', 'projected', 'emerging']):
...
def get_openai_embedding(text: str) -> List[float]:
    ...
    try:
        ...
        tenant = os.environ.get('AR_TENANT_ID', 'maxdemo')
        ...
        for i, source in enumerate(loaded_sources):
            ...
            try:
                ...
                if similarity >= float(match_threshold):
                    ...
                    # Generate knowledge base URL
                    doc_id = "50f685d2-0617-4334-97d3-509b62d20095"
                    source_copy['url'] = f"https://maxdemo.staging.answerrocket.com/apps/system/knowledge-base/{doc_id}#page={source_copy['chunk_index']}"
...
narrative_prompt = """
You are analyzing pharmaceutical industry market intelligence documents. Extract and summarize ALL relevant information from the provided sources to answer the user's question. Be comprehensive and specific.

IMPORTANT INSTRUCTIONS:
1. Extract SPECIFIC details like spending figures, growth rates, therapeutic areas, regional data, and market projections
...
4. For spending questions, look for dollar amounts, growth percentages, and forecasts
5. For therapeutic area questions, identify specific therapy classes, disease areas, and treatment trends
6. For market questions, note regional differences, emerging vs. developed markets, and policy impacts
7. For medicine type questions, distinguish between specialty/branded/generic/biosimilar medicines
